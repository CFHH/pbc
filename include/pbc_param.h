...
*/
PBC_DECLSPEC_EXPORT int pbc_param_init_set_str(pbc_param_t par, const char *s);
...
Returns 0 if successful, 1 otherwise.
*/
PBC_DECLSPEC_EXPORT int pbc_param_init_set_buf(pbc_param_t par, const char *s, size_t len);

/*@manual param
Write pairing parameters to ''stream'' in a text format.
*/
/*PBC_DECLSPEC_EXPORT*/ static inline void pbc_param_out_str(FILE *stream, pbc_param_ptr p) {
...
*/
/*PBC_DECLSPEC_EXPORT*/ static inline void pbc_param_clear(pbc_param_ptr p) {
  p->api->clear(p->data);
}
