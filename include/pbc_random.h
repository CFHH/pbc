...
*/
PBC_DECLSPEC_EXPORT void pbc_random_set_file(char *filename);

/*@manual pbcrandom
Uses a determinstic random number generator, seeded with 'seed'.
*/
PBC_DECLSPEC_EXPORT void pbc_random_set_deterministic(unsigned int seed);

/*@manual pbcrandom
Uses given function as a random number generator.
*/
PBC_DECLSPEC_EXPORT void pbc_random_set_function(void (*fun)(mpz_t, mpz_t, void *), void *data);

/*@manual pbcrandom
Selects a random 'z' that is less than 'limit'.
*/
PBC_DECLSPEC_EXPORT void pbc_mpz_random(mpz_t z, mpz_t limit);
