...
struct pairing_s {
  ...
  void (PBC_STDCALL *phi)(element_ptr out, element_ptr in, struct pairing_s *pairing); //isomorphism G2 --> G1
  void (PBC_STDCALL *map)(element_ptr out, element_ptr in1, element_ptr in2,
      struct pairing_s *p);
  void (PBC_STDCALL *prod_pairings)(element_ptr out, element_t in1[], element_t in2[], int n_prod,
  ...
  int (PBC_STDCALL *is_almost_coddh)(element_ptr a, element_ptr b,
      element_ptr c, element_ptr d,
      struct pairing_s *p);
  void (PBC_STDCALL *clear_func)(struct pairing_s *);
  void (PBC_STDCALL *pp_init)(pairing_pp_t p, element_t in1, struct pairing_s *);
  void (PBC_STDCALL *pp_clear)(pairing_pp_t p);
  void (PBC_STDCALL *pp_apply)(element_t out, element_t in2, pairing_pp_t p);
  void (PBC_STDCALL *finalpow)(element_t e);
  void (PBC_STDCALL *option_set)(struct pairing_s *, char *key, char *value);
  void *data;
};
