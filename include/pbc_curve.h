...
PBC_DECLSPEC_EXPORT void field_init_curve_b(field_ptr f, element_ptr b, mpz_t order, mpz_t cofac);

// Initialize a subgroup of points on the curve with the given j-invariant.
PBC_DECLSPEC_EXPORT void field_init_curve_j(field_t f, element_ptr j, mpz_t order, mpz_t cofac);

// Initialize a subgroup of points on the curve Y^2 = X^3 + a X + b.
PBC_DECLSPEC_EXPORT void field_init_curve_ab(field_ptr f, element_ptr a, element_ptr b, mpz_t order, mpz_t cofac);
...
PBC_DECLSPEC_EXPORT void field_reinit_curve_twist(field_t f);

// Compute trace of Frobenius at q^n given trace at q.
PBC_DECLSPEC_EXPORT void pbc_mpz_trace_n(mpz_t res, mpz_t q, mpz_t trace, int n);

// Given q, t such that #E(F_q) = q - t + 1, compute #E(F_q^k).
PBC_DECLSPEC_EXPORT void pbc_mpz_curve_order_extn(mpz_t res, mpz_t q, mpz_t t, int k);

PBC_DECLSPEC_EXPORT void field_init_curve_with_map(field_ptr cnew, field_ptr c,
  field_ptr dstfield, fieldmap map);

PBC_DECLSPEC_EXPORT void field_init_curve_ab_map(field_t cnew, field_t c,
  fieldmap map, field_ptr mapdest,
  mpz_t ordernew, mpz_t cofacnew);

PBC_DECLSPEC_EXPORT void field_curve_use_random_solvefory(field_ptr f);

PBC_DECLSPEC_EXPORT void field_curve_set_quotient_cmp(field_ptr c, mpz_t quotient_cmp);
...
PBC_DECLSPEC_EXPORT element_ptr curve_x_coord(element_t e);
PBC_DECLSPEC_EXPORT element_ptr curve_y_coord(element_t e);
PBC_DECLSPEC_EXPORT element_ptr curve_a_coeff(element_t e);
PBC_DECLSPEC_EXPORT element_ptr curve_b_coeff(element_t e);
PBC_DECLSPEC_EXPORT element_ptr curve_field_a_coeff(field_t f);
PBC_DECLSPEC_EXPORT element_ptr curve_field_b_coeff(field_t f);

PBC_DECLSPEC_EXPORT void curve_from_x(element_ptr e, element_t x);
PBC_DECLSPEC_EXPORT void curve_set_si(element_t R, long int x, long int y);
PBC_DECLSPEC_EXPORT void curve_set_gen_no_cofac(element_ptr a);
